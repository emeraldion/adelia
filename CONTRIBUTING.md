...
You can find in-depth instructions to install the correct Node version, MySQL server, and to set up the database in our [README](https://github.com/emeraldion/adelia/blob/master/README.md#development).

## Reporting a bug

So you've found a bug, and want to help us fix it? Before filing a bug report, please double-check the bug hasn't already been reported. You can do so [on our issue tracker](https://github.com/emeraldion/adelia/issues?q=is%3Aissue+is%3Aopen+label%3Abug). If something hasn't been raised, you can go ahead and create a new issue with the following information:

* Which API did cause an error?
* What DB adapter did you use?
...
We've got a few open issues and are always glad to get help on that front. You can view the list of issues [here](https://github.com/emeraldion/adelia/issues). Most of the issues are labelled, so you can use the labels to get an idea of which issue could be a good fit for you. (Here's [a good article](https://medium.freecodecamp.com/finding-your-first-open-source-project-or-bug-to-work-on-1712f651e5ba) on how to find your first bug to fix).
...
- **i18n**: Issues related to localization or translation are marked with these labels. They can range from missing translations for a specific language to grammatical issues and cultural differences. These will usually require a specialist in a non-English language to close.
...
- **mysql**: This means that the issue is specific to the MySQL database. It could be an issue with the adapter code, or the SQL dialect that the library uses to communicate with the database.
- **sqlite**: This means that the issue is specific to the SQLite database. It could be an issue with the adapter code, or the SQL dialect that the library uses to communicate with the database.
