...
function singularize(term) {
	_.forEach(irregular_nouns, function(plural, singular) {
		if (_.endsWith(term, plural)) {
			return term.substr(0, term.length - plural.length) + singular;
		}
	});
...
function joined_lower_to_camel_case(text) {
	return text.replace(/(^[a-z])|_([a-z])/, function (match, p1, p2) {
		return p1.concat(p2).toUpperCase();
	});
}
