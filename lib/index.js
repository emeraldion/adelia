...
_.assign(Model.prototype, {
   ...
   * @param [opts.force] if set to true, forces an insert when primary key is set
   * @param [opts.ignore] if set to true, tries an insert ignore
  ...
  save: function(opts) {
    ...
    return this.initialize()
      .then(function() {
        ...
        if (!_.isUndefined(self.values[self.primaryKey]) && _.isUndefined(opts.force)) {
          query = 'UPDATE `{1}` SET ';
        ...
        query = (!_.isUndefined(opts.ignore) ? 'INSERT IGNORE' : 'INSERT') + ' INTO `{1}` (';
        for (let i = 0; i < nonempty.length; i++) {
