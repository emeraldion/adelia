...
_.assign(Model.prototype, {
  ...
  save: function() {
    ...
    if (!_.isUndefined(this.values[this.primaryKey]) && _.isUndefined(this._force_create)) {
      query = "UPDATE `{1}` SET ";
      for (let i = 0; i < nonempty.length; i++) {
        query += sprintf("`%s` = '%s'", nonempty[i], this.values[nonempty[i]]);
        if (i < nonempty.length - 1) {
          query += ", ";
        }
      }
      query += sprintf(" WHERE `%s` = '{2}' LIMIT 1", this.primaryKey);
      conn.prepare(query,
        this._getTableName(),
        this.values[this.primaryKey]);

      return conn.exec().then(function() {
        Db.closeConnection(conn);
    ...
    query += ") VALUES (";
    for (let i = 0; i < nonempty.length; i++) {
      query += sprintf("'%s'", this.values[nonempty[i]]);
      if (i < nonempty.length - 1) {
        query += ", ";
      }
    }
    ...
    return conn.exec().then(function(result) {
