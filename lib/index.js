...
const _ = require('lodash'),
  ...
  tableName_to_class_name = utils.tableName_to_class_name,
  tableName_to_foreign_key = utils.tableName_to_foreign_key;
 ...
 *  by setting explicitly <tt>tableName</tt> and <tt>foreign_key</tt> to a value of your choice.
 */
...
_.assign(Model.prototype, {
  ...
  belongsTo: function(tableName) {
    let classname = get_class(this);
    let columns = this._getColumns(classname);
    let ownerclass = tableName_to_class_name(tableName);
    let owner = new global[ownerclass]();
    if (in_array(tableName_to_foreign_key(tableName), columns)) {
      owner.findById(this.values[tableName_to_foreign_key(tableName)]);
    } else if (in_array(owner.foreignKeyName, columns)) {
  ...
  hasMany: function(tableName, params) {
    ...
    let childclass = tableName_to_class_name(tableName);
    let obj = new global[childclass]();
    let fkey = this.getForeignKeyName();
    if (!_.isUndefined(params['where_clause'])) {
      params['where_clause'] = sprintf("(%s) AND `%s` = '%s' ", params['where_clause'], fkey, this.values[this.primaryKey]);
    } else {
      params['where_clause'] = sprintf("`%s` = '%s' ", fkey, this.values[this.primaryKey]);
    }
  ...
  hasAndBelongsToMany: function(tableName, params) {
    ...
    let peerclass = tableName_to_class_name(tableName);
  ...
  hasOne: function(tableName) {
    childclass = tableName_to_class_name(tableName);
    obj = new global[childclass]();
    fkey = this.getForeignKeyName();
    children = obj.findAll({
        'where_clause': sprintf("`%s` = '%s'", fkey, this.values[this.primaryKey]),
        'limit': 1
      });
    if (children.length > 0) {
      child = children[0];
  ...
  findAll: function(params) {
    ...
    if (_.isEmpty(params['order_by'])) {
      params['order_by'] = sprintf('`%s` ASC', this.primaryKey);
    }
