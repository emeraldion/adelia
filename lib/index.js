...
_.assign(Model.prototype, {
  ...
  belongsTo: function(tableName) {
    const classname = get_class(this);
    const columns = this._getColumns(classname);
    const ownerclass = table_name_to_class_name(tableName);
    const owner = new global[ownerclass]();
  ...
  hasMany: function(tableName, params) {
    ...
    const childclass = table_name_to_class_name(tableName);
    const obj = new Model.create(childclass)();
    const fkey = this.getForeignKeyName();
    const self = this;
    ...
    return obj.findAll(params)
      .then(function(children) {
        if (children.length > 0) {
          ...
          self.values[tableName] = children;
        }
  ...
  hasAndBelongsToMany: function(tableName, params) {
    ...
    const peerclass = table_name_to_class_name(tableName);
    const peer = new Model.create(peerclass)();
    const fkey = this.getForeignKeyName();
    const peer_fkey = peer.getForeignKeyName();
    const self = this;
    ...
    conn.prepare("SELECT * FROM `{1}` WHERE `{2}` = '{3}'",
        relation_table, fkey, this.values[this.primaryKey])
      .then(function() {
        return conn.exec();
      })
    ...
    return obj.findAll({
      ...
      .then(function(children) {
        if (children.length > 0) {
          child = children[0];
          child.values[singularize(self.tableName)] = self;
          self.values[singularize(tableName)] = child;
        }
  ...
  delete: function(optimize) {
    ...
    if (!_.isUndefined(this.values[this.primaryKey])) {
      return conn.prepare(sprintf("DELETE FROM `{1}` WHERE `%s` = '{2}' LIMIT 1", this.primaryKey),
            this._getTableName(), this.values[this.primaryKey])
        .then(function() {
          return conn.exec();
        })
        .then(function() {
          if (optimize) {
            return conn.prepare('OPTIMIZE TABLE `{1}`', self._getTableName())
              .then(function() {
                return conn.exec();
              });
          }
