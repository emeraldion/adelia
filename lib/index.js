...
function Model(_values) {
	...
	const initialized = this._is_initialized(classname);
	let columns;
	...
	if (!_.isEmpty(_values)) {
		...
		_.forEach(_values, function(val, key) {
			...
			if (keyexists) {
				self.values[key] = (get_magic_quotes_gpc()) ? stripslashes(val) : val;
			}
		});
	}
	this.init(_values);
...
_.assign(Model.prototype, {
	 ...
	 */
	belongs_to: function(table_name) {
		let classname = get_class(this);
		let columns = this._get_columns(classname);
	 ...
	 *	@see find_all
	 */
	has_many: function(table_name, params) {
		params = params || [];
	 ...
	 *	@see find_all
	 */
	has_and_belongs_to_many: function(table_name, params) {
		params = params || [];
	...
	find_by_id: function(id) {
		...
		if (conn.num_rows() > 0) {
			...
			this._add_to_pool(classname, id, this);
